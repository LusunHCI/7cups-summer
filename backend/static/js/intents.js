...
function send(message) {
    ...
    $.ajax({
        ...
        success: function(json) {
            console.log(json);
            intent=json['intent'];
            appendIntentsAndScores(intent, message);
        }
    });
...
function appendIntentsAndScores(intent, message) {
    ...
    var editIntents='<div class="input-field"> <label for="userFeedback">Please help us correct this intent</label> <input id="userFeedback ' +message_count.toString()+'" class="userFeedback" type="text" name="userFeedback'+message_count.toString()+'""> </div>';
    $(editIntents).appendTo(".feedback");
}
        ...
        if (response.length < 1) {
            ...
            for (i = 0; i < response.length; i++) {
                ...
                if (response[i].hasOwnProperty("text")) {
                    ...
                    $(BotResponse).appendTo(".chats").hide().fadeIn(1000);
                    message_count++;
                    var botAction='<p class="feedbackMsg">' + response[i].text + '</p> <p>The corresponding action is "stressor_introduction"</p> <p> Do you think this action need to be changed? </p>';
                    $(botAction).appendTo(".feedback");
                    var choiceButton= '<input type="text" class="hiddenInput" style="width:1px"> <button class="yesedit btn" type="button" id="yesedit" style="background-color:white;border-radius:30px; border: 2px solid #5a17ee; color: #5a17ee"> Yes </button>  <button class="noedit btn" id="noedit"  type="button" style="background-color:white;border-radius:30px; border: 2px solid #5a17ee; color: #5a17ee"> No </button>';
                    $(choiceButton).appendTo(".feedback");
                    var editAction='<div class="input-field"> <label for="userFeedback">Please help us correct this action</label> <input id="userFeedback ' +message_count.toString()+'class="userFeedback" type="text" name="userFeedback'+message_count.toString()+'""> </div>';
                    $(editAction).appendTo(".feedback");
                }
