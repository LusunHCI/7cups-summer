...
responseDict={
	"utter_greeting": "Hello. Are you there?"
    ...
    ,"respond_open_question":["we were together for 6 months and then she broke up with me 2 weeks ago.","I am 22 years old and my ex is 20.","She told me she broke up with me because she wasn't interested in me.","I just can't get over it. Like how can she go from saying she's falling for me to saying she never had feelings for me at all.","That's the thing. I don't really know because she kept changing her mind about me, first telling me she LOVED me and then dumping me, saying she NEVER FELT THE SAME.","I'm still in love with her plain and simple despite my anger for her actions.","I've been trying to avoid her, but she works at this cafe I frequent","Yeah sometimes at the cafe she works at that I usually go to before class. "]
}
currentShow=0;
...
function appendActions(botmessage,msg_type,msgid) {
    if(msg_type==0) {
     ...
     if(scores[i]['action'].substr(0,7)=='respond') {
                // console.log(responseDict[scores[i]['action']]);
        ...
        $('select').formSelect();
        var otherResponse="<div class='otherResponse' id='otherResponse"+msgid+"'><p>What other ways could the chatbot respond to this intent</p> <textarea class='materialize-textarea' name='other"+msgid+"' id='other"+msgid+"' style='float:left;width:100%;'></textarea>  </div>";
    ...
    if (response.length < 1) {
        ...
        for (i = 0; i < response.length; i++) {
            ...
            if (response[i].hasOwnProperty("text")) {
                var botTyping = '<div id="chatbotTyping'+message_count.toString()+'"><img class="botAvatar" src="../static/img/sara_avatar.png"/><div class="botTyping">' + '<div class="bounce1"></div>' + '<div class="bounce2"></div>' + '<div class="bounce3"></div>' + '</div> </div>'
                $(botTyping).appendTo(".chats");
                $('#chatbotTyping'+message_count.toString()).hide(0).delay(1000*i).show(0).delay(1000).hide(0);
                ...
                $(BotResponse).appendTo(".chats").hide(0).delay(1000*(i+1)).fadeIn(1000);
                message_count++;
                setTimeout(function(){ scrollToBottomOfResults() }, i*1000);
